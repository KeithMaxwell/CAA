...
from behaviors import HALLUCINATION, get_analysis_dir, HUMAN_NAMES, get_steering_vector, ANALYSIS_PATH
from utils.helpers import get_model_path, model_name_format, set_plotting_settings
from tqdm import tqdm

set_plotting_settings()

def get_caa_info(behavior: str, model_size: str):
    all_vectors = []
    n_layers = 32
...
def plot_per_layer_similarities(model_size: str, behavior: str):
    analysis_dir = get_analysis_dir(behavior)
    caa_info = get_caa_info(behavior, model_size)
    all_vectors = caa_info["vectors"]
    # cosine_similarity 内部会用到 clamp_min 之类的算子，CPU 端对 fp16 支持不全（我们在部署模型的时候使用了fp16减少显存占用
    # 放到gpu上去算
    all_vectors = [v.to("cuda") for v in all_vectors]
...
if __name__ == "__main__":
    plot_per_layer_similarities("8b", HALLUCINATION)
